...
The supplied 16x16 journal-and-quill Aseprite artwork is exported losslessly to PNG and mapped through Nexo as WRITABLE_BOOK custom model data 815002. EnthusiaAdvancements pilot 1.0.0-pilot.2 adds optional per-node custom model data while preserving the existing Material-only Node constructor. The Dear Diary node uses the custom icon; other diary nodes keep their distinct vanilla icons. Focused diary/layout tests pass 18 tests and the full EnthusiaTags verification passes 203 tests.
