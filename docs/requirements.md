...
- REQ-051: WHEN the DiaryKeeper branch is rendered THE SYSTEM SHALL use the supplied journal-and-quill artwork as the Dear Diary advancement icon through WRITABLE_BOOK custom model data 815002, while retaining vanilla fallback behavior when the resource pack is absent.
