...
class DiaryProgressTest {
    @Test void historicalCompletionsAreSilent() {
        ...
        var update = progress.observe(new Stats(true, 25, true, 10, 1, 1, 1));
    ...
    @Test void liveCrossingsCelebrateOnce() {
        var progress = new DiaryMilestoneProgress();
        progress.observe(new Stats(true, 24, false, 9, 0, 0, 0));

        var update = progress.observe(new Stats(true, 25, true, 10, 1, 1, 1));
        assertEquals(Set.of(
            "diary/prolific_writer",
            "diary/signed_sealed_delivered",
            "diary/stubborn",
            "diary/void_walker",
            "diary/nice_try",
            "diary/finders_keepers"
        ), update.celebrate());
        assertTrue(progress.observe(new Stats(true, 25, true, 10, 1, 1, 1))
            .celebrate().isEmpty());
    }

    @Test void unavailableAndRegressedEvidenceRetainsProgress() {
        var progress = new DiaryMilestoneProgress();
        var historical = progress.observe(new Stats(true, 12, false, 5, 0, 0, 0));

        assertEquals(historical.progress(), progress.observe(null).progress());
        var regression = progress.observe(new Stats(false, 1, false, 0, 0, 0, 0));
    ...
    @Test void invalidEvidenceIsRejected() {
        assertThrows(IllegalArgumentException.class,
            () -> new Stats(false, -1, false, 0, 0, 0, 0));
        assertThrows(IllegalArgumentException.class,
            () -> new Stats(false, 0, false, -1, 0, 0, 0));
    }
}
